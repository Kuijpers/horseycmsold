<?php
	...
	$page_id = "Breeding terms"; // De tekst tussen de " " moet je veranderen voor de pagina titel ( <title></title> )
        	...
        	<div class="body">
				<div class="admin_menu">
        			<?php
						if ( $page_id !== "Main")
							{
								include ('includes/breeding_menu.html');
							}
	   ...
	   !$errors)
		{
			...
			header ('location: settings_breedterms.php');
		}
...
$sql1 = "INSERT INTO picture (image,stamp) VALUES ('" . $image_name . "','" . $stamp . "')";
		...
		if ($errors	== 0)
			{
				header ('location: settings_breedterms.php?action=member&process=list');
			}
		else
			{
					echo	"<table>";
...
case 'forbreeding':
?>	
<form name="forbreeding" enctype="multipart/form-data" action="?lang=<?php echo $lang ?>&amp;action=member&amp;process=forbreedcheck" method="post">
<table>
	<tr class="listing">
		<th colspan="4">
        	<b>
				<?php echo "Stallions that aren't available for breeding"; ?>
            </b>
        </th>
	</tr>
    	...
    	<?php
			...
			$sql ="SELECT * FROM ".$tablename." WHERE breed=0 AND gender=1 ORDER BY id ASC";
    				$data = mysql_query($sql);
					if (mysql_num_rows($data)==0)
					echo "No records available";
					else
...
<form name="addforsale" enctype="multipart/form-data" action="?lang=<?php echo $lang ?>&amp;action=member&amp;process=forbreedadd" method="post">
<table>
	<tr>
    	<td>
        	Name of the horse
    ...
    <tr>
    	<th colspan="4">
        	Are you sure you want to use this stallion for breeding
        </th>
        <td>
        	:
        </td>
        <td>
        	<input name="surebreed" type="radio" value="1" /> Yes <input name="surebreed" type="radio" value="0" checked="checked" /> No
        </td>
    </tr>
    <tr>
    	<td>
        	Breedprice for the horse
        </td>
        <td>
        	:
        </td>
        <td>
        	&#8364; <input name="breedprice" type="text" size="5" />,--
        </td>
        <td>
    ...
    <tr>
    	<td>
        	...
        	<input name="submit" type="submit" value="Add stallion to breedinglist" />
        </td>
    </tr>
</table>
</form>
...
<?php
								$horse_id	=	$_POST['horse_id'];
								$surebreed	=	$_POST['surebreed'];
								$breedprice	=	$_POST['breedprice'];
						...
						if ($surebreed !== '1')
						{
							header("Refresh: 3; URL=\"?action=member&process=forbreedcheck&lang=$lang&horse_id=$horse_id\"");
						
								echo 	"<table>";
								echo	"<tr><td>";
								echo 	"You didn't check the button: Are you sure you want to use this stallion for breeding ?";
								echo	"</td></tr>";
								echo	"<tr><td><i>";
						...
						if ($request == "1"){$breedprice	= "request";} // wanneer de prijs op verzoek is anders gebruiken we de prijs die aangegeven is
							
							if (empty($breedprice))
							{
								header("Refresh: 3; URL=\"?action=member&process=forbreedcheck&horse_id=$horse_id\"");
						
								echo 	"<table>";
								echo	"<tr><td>";
								echo 	"You didn't fill in a breedprice";
								echo	"</td></tr>";
								echo	"<tr><td><i>";
								echo	"You will be redirected in 3 seconds.";
								echo	"</i></td></tr>";
							...
							$sql = "UPDATE horses SET breed = '".$surebreed."', breed_price = '".$breedprice."'WHERE id=".$horse_id."";
	
							//echo $sql;
								mysql_query($sql) or die(mysql_error());
								
								header("Refresh: 0; URL=\"?action=member&process=forbreeding\"");
...
<?php
			...
			$sql ="SELECT * FROM ".$tablename." WHERE breed=1 ORDER BY id ASC";
    				$data = mysql_query($sql);
					if (mysql_num_rows($data)==0)
					{
...
<tr>
	<th colspan="3">
    	<h3>
    		Stallions not available for breeding
        </h3>
    </th>
</tr>
...
case 'changebreeding':
?>
<form name="multiunbreed" enctype="multipart/form-data" action="?lang=<?php echo $lang ?>&amp;action=member&amp;process=multiunbreed" method="post">	
<table>
<tr>
	<th colspan="4">
    	<h3>
    		Stallions that are available for breeding
        </h3>
    </th>
</tr>
					...
					while($info = mysql_fetch_array( $data ))
						{
							...
							$breed_price	=	stripslashes($breed_price);
?>
							
						
                        	<tr class="listing">
                            	<td>
                                	<input name="unbreed[]" type="checkbox" value="<?php echo $id ; ?>" />
                                </td>
                    			<td>
                        			<?php echo $fullname; ?>
                        		</td>
                                ...
                                <td>
                                &nbsp;
      								<a href="?action=member&amp;process=editbreeding&amp;regid=<?php echo $id  ?>">
      								<img src="images/edit.gif" alt="<?php echo $ADM['changeinfo']; ?>" title="<?php echo $ADM['changeinfo']; ?>" />
      								</a>
                                </td>
                   		 </tr>
...
<tr>
	<th colspan="2">
    	<input name="submit" type="submit" value="Change to not breeding" />
    </th>
</tr>
<?php
					}		
...
<form name="multiunsold" enctype="multipart/form-data" action="?lang=<?php echo $lang ?>&amp;action=member&amp;process=forbreedcheck" method="post">	
<table>
<tr>
	<th colspan="2">
    	<h3>
...
<tr>
	<th colspan="2">
    	<input name="submit" type="submit" value="Change to breeding" />
    </th>
</tr>
<?php
					}		
			...
			$sql ="SELECT * FROM ".$tablename." WHERE id=".$regid." ";
					...
					else
					{
						$info = mysql_fetch_array( $data );
							...
							$fullname 	= 	$info['fullname'];
							$stamp 		= 	$info['stamp'];
							$breedprice	=	$info['breed_price'];
							
							$fullname	=	stripslashes($fullname);
							$breedprice	=	stripslashes($breedprice);
					}
...
<form name="changebreeding" enctype="multipart/form-data" action="?lang=<?php echo $lang ?>&amp;action=member&amp;process=changebreed" method="post">
<table>    
    <tr>
    	<td>
        	Remove from breed list :
        </td>
        <td>
    ...
    <tr>
		...
		?>
        <td>
        		<input name="breedprice" type="radio" value="0"/> On request 
                <input name="breedprice" type="radio" value="set" checked="checked" /> Set price 
                &#8364;
            	<input name="breedvalue" type="text" value="<?php echo $breedprice ; ?>" size="5" />
        		,--
        </td>
        <?php
			}
		...
		?>
			<td>
        		<input name="breedprice" type="radio" value="0" checked="checked"/> On request 
                <input name="breedprice" type="radio" value="set" /> Set price 
                &#8364;
            	<input name="breedvalue" type="text" size="5" />
        		,--
            </td>
        <?php
			}
...
<?php
	...
	if ($remove == '1')
		{
			$sql = "UPDATE ".$tablename." SET breed = 0 WHERE id = ".$id.";";
	...
	else
		{
								if ($breedprice !== 'set')
						{
							$sql = "UPDATE ".$tablename." SET breed_price = '".$request."' WHERE id = ".$id.";";
						...
						{
							$sql = "UPDATE ".$tablename." SET breed_price = ".$breedvalue." WHERE id = ".$id.";";
...
header("Refresh: 0; URL=\"?action=member&process=changebreeding\"");
?>
...
<?php
  ...
  if (empty($_POST['unbreed']))
  	{
		$_POST['unbreed'] = "";
	}
	else
	{
		$_POST['unbreed'] = $_POST['unbreed'];	
	}
  
  
  $aUnbreed = $_POST['unbreed'];
  if(empty($aUnbreed)) 
  {
	  echo "<table><tr><td>";
	  echo "<h3>Error</h3>";
	  echo "</td></tr><tr><td>";
	  echo "You didn't select any stallion that is not available for breeding anymore.";
	  echo "</td></tr><tr><td>";
	  echo "You will be redirected automatically.";
	  echo "</td></tr></table>";
	  header("Refresh: 2; URL=\"?action=member&process=changebreeding\"");
  } 
  else 
  {
    $N = count($aUnbreed);
    ...
    {
      //echo($aSold[$i] . " ");
	  $sql =  "UPDATE ".$tablename." SET breed = 0  WHERE id = ".$aUnbreed[$i].";";
	  //echo	$sql;
	  mysql_query($sql);
